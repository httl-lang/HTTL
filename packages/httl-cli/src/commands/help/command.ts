...
export class HelpCommand implements IProgramCommand {
  ...
  public async run(): Promise<void> {
    ...
    console.log(chalk.white(`     httl get httl.dev/api/users`));
    console.log();
    console.log(chalk.gray('    Simple GET request with a basic .httl file with content: `@base: httl.dev/api`'));
    process.stdout.write(chalk.gray(`    (more about the default .httl file at `));
    process.stdout.write(chalk.blueBright(`httl.dev/docs/guide/module#default-httl-file`));
    console.log(chalk.gray(`)`));
    console.log(chalk.white(`     httl get users`));
    console.log();
    console.log(chalk.gray(`    GET request with headers`));
    console.log(chalk.white(`     httl get httl.dev/api/users Authorization:"Bearer token" Cache-Control:private`));
    console.log();
    console.log(chalk.gray(`    POST request with a JSON body`));
    console.log(chalk.white(`     httl post httpbin.org/anything '{ name: "John" }'`));
    console.log();
    console.log(chalk.gray(`    PUT request with form data; JSON payload will be converted to form data`));
    console.log(chalk.white(`     httl put httpbin.org/anything --formdata '{ name: "John" }'`));
    console.log();
    console.log(chalk.gray(`    POST request with a URL-encoded body`));
    console.log(chalk.white(`     httl post httpbin.org/anything --urlencoded '{ name: "John" }'`));
    console.log();
    console.log(chalk.gray(`    POST request with a raw body`));
    console.log(chalk.white(`     httl post httpbin.org/anything 'name=John&age=30'`));
    console.log();
    console.log(chalk.gray(`    POST request with a binary file`));
    console.log(chalk.white(`     httl post httpbin.org/anything --bin ./file.bin`));
    console.log();
    console.log(chalk.gray(`    POST request with a payload from the input stream; HTTP will automatically detect the content type.`));
    console.log(chalk.white(`     httl post httpbin.org/anything < ./payload.json`));
    console.log();
    console.log();
    ...
    console.log(chalk.white(`     @base: httl.dev/api`));
